  ...
  Column {
    ...
    BorderSurface {
      ...
      QQC.Popup {
        ...
        // clipping ancestor. Position, side (below vs above), and available
        // height are recomputed on open and any time the trigger's geometry
        // changes, since a binding on mapToItem alone won't reliably
        // re-evaluate when ancestors scroll or resize.
        ...
        function reposition() {
          if (!parent) return
          var base = trigger.mapToItem(parent, 0, 0)
          var belowY = base.y + trigger.height + Style.spacing.xxs
          _availableBelow = Math.max(0, _windowHeight - belowY - Style.space(12))
          var idealH = Math.min(_idealContent, _maxRowsHeight)
          _above = idealH + Style.spacing.xxs > _availableBelow
          _anchorY = _above ? base.y - idealH - Style.spacing.xxs : belowY
          _anchorX = base.x
          if (base.x + trigger.width > _windowWidth) _anchorX = Math.max(0, _windowWidth - trigger.width)
        }

        x: _anchorX
        y: _anchorY
        width: trigger.width
        // When flipping above use the full desired height; when below clamp
        // to whatever fits, otherwise the popup overflows the window.
        implicitHeight: _above ? idealHeight : Math.min(_availableBelow, _idealContent, _maxRowsHeight)
        property real idealHeight: Math.min(_idealContent, _maxRowsHeight)
